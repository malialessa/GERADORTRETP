...
def apply_basic_markdown_to_docs_requests(markdown_content: str) -> List[Dict]:
    ...
    for line in lines:
        ...
        if line_stripped == "<NEWPAGE>":
            ...
            current_index += 1 # Page break consome 1 caractere no índice
            continue

        if not line_stripped: # Linha vazia, insere nova linha (parágrafo)
        ...
        if line_stripped.startswith('## '):
            ...
            text = line_stripped[2:]
            requests.append({"insertText": {"location": {"index": current_index}, "text": text + "\n"}})
            # Aplica bullet ao parágrafo que acabou de ser inserido.
            # O range para createParagraphBullets deve cobrir o parágrafo.
            # +1 para o \n é importante para que o bullet seja aplicado corretamente.
            requests.append({
        ...
        elif "**" in line_stripped: # Lida com negrito
            parts = []
            last_pos = 0
            # Itera sobre todas as ocorrências de **texto** na linha
            for match in re.finditer(r'\*\*(.*?)\*\*', line_stripped):
                # Adiciona texto antes da parte em negrito (se houver)
                if match.start() > last_pos:
                    parts.append({'text': line_stripped[last_pos:match.start()], 'bold': False})
                # Adiciona a parte em negrito
                parts.append({'text': match.group(1), 'bold': True})
                last_pos = match.end()
            # Adiciona texto após a última parte em negrito (se houver)
            if last_pos < len(line_stripped):
                parts.append({'text': line_stripped[last_pos:], 'bold': False})

            # Insere as partes no documento
            for part in parts:
            ...
            # Adiciona uma nova linha ao final do parágrafo que continha negrito
            requests.append({"insertText": {"location": {"index": current_index}, "text": "\n"}})
            current_index += 1
...
"""
    ...
    try:
        ...
        if generated_text:
            # Tenta extrair JSON de dentro de ```json ... ```, caso o LLM o retorne assim
...
async def generate_etp_tr_endpoint(
    ...
    try:
        ...
        # Combina conteúdo ETP e TR, com uma quebra de página explícita
